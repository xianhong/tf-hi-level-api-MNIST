...
def _float32_feature (image):
    return tf.train.Feature(float_list=tf.train.FloatList(value=image[0].tolist()))
  ...
  for _ in range(num_tests):
    ...
    example = tf.train.Example(
                    features=tf.train.Features(feature=
                        {'x': _float32_feature(image)})
                )
    request.inputs['inputs'].CopyFrom(
        tf.contrib.util.make_tensor_proto([example.SerializeToString()]))
